...
typedef struct _THREAD_DATA {
	...
	HANDLE				hAccountFile;  // handle to "account" file
...
INT _tmain(INT argc, LPTSTR argv[])
{
	...
	//semaphore intiialization
	semaphore = CreateSemaphore(NULL, 1, numThread, NULL); // so that the first thread that waits take the control and the other wait
	...
	for (int iThread = 0; iThread < numThread; iThread++) {
		...
		arrayOfThreadData[iThread].sem = &semaphore;
	...
	//close the handle referst to the mutex
...
DWORD WINAPI work(PVOID param) {
	...
	while (ReadFile(hOperationFile, &fileData, sizeof(FILE_DATA), &nIn, NULL) > 0 && nIn > 0) {
		...
		//WAIT FOR only one at a time
		WaitForSingleObject(data->nameOperationFile, INFINITE);

		//R E C O R D   U P D A T I N G
		//here i have to set the position in "account" file in order to change the amount of money
		ReadFile(data->hAccountFile, &fileDataApp, sizeof(FILE_DATA), &nIn, &overlapped);
	   ...
	   //write the updated ecord on the file in the same position in which has been read
		WriteFile(data->hAccountFile, &fileDataApp, sizeof(FILE_DATA), &nOut, &overlapped);

		// send a post in order to go haead the other threads wich are waiting
